...
- **15fps** — 움직임이 아주 느려서 육안 차이가 없고, 파일만 가벼워집니다.
...
브라우저는 WebM(약 18KB)을 먼저 쓰고, WebM을 못 트는 구형 사파리·iOS만
...
Remotion은 프레임을 여러 개씩 병렬로 그리기 때문에 CPU 코어 수를 그대로 탑니다.
기준점으로, **4코어 2.1GHz**에서 이 프로젝트의 기본 영상(14초 = 420프레임,
1080×1920)을 뽑는 데 **약 29초**가 걸렸습니다. 번들링과 인코딩까지 포함한 시간입니다.

미니PC 등급별로 대략 이런 차이가 납니다.

| CPU 등급 | 예 | 기본 영상 렌더 | 권장 옵션 |
|---|---|---|---|
| 저전력 4코어 | Intel N100 / N95 | 1~2분 | `--concurrency=2` |
| 모바일 8코어 | Ryzen 7 7735HS / 8745HS | 20초 안팎 | 기본값 그대로 |

저전력 칩이라고 못 쓰는 건 아닙니다. 다만 램이 8GB 이하면 코어를 다 쓰다가
메모리가 모자라 중간에 죽을 수 있습니다. 그럴 땐 동시 실행 수를 낮추세요.
느려지는 대신 안정적으로 끝납니다.
...
미리보기(`npm run studio`)는 렌더보다 훨씬 가볍습니다. 문구 다듬는 작업은
사양과 관계없이 쾌적하니, 스튜디오에서 충분히 확인한 뒤 마지막에 한 번만
렌더하는 흐름을 권합니다.
