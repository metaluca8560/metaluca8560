...
## 4-3. 크레딧 소진 순서 (보유 470)

컷 1부터 순서대로 뽑으면 안 된다. 중간에 크레딧이 떨어지면 **쓸 수 없는 반쪽짜리**가 남는다. 위험한 것부터, 그리고 "여기까지만 있어도 완성품"이 되는 순서로 간다.

> ⚠️ 힉스필드의 생성당 크레딧 소모량은 모델·길이·해상도에 따라 다르고 나는 정확한 값을 모른다. **1단계 첫 생성 후 실제 차감량을 확인해서 아래 배분을 다시 계산할 것.**

### 4단계 소진 계획

| 단계 | 내용 | 배분 | 왜 이 순서인가 |
|---|---|---:|---|
| **1. 일관성 + 난이도 테스트** | **컷 2(석재 고래) + 컷 6(생명체 고래)** | ~20% | **여기서 막히면 기획 자체를 바꿔야 한다.** 컷 1은 평면 각석이라 비교 대상이 안 되므로 테스트 쌍에서 제외. 컷 2는 난이도도 최상이라 두 가지를 한 번에 거른다 |
| **2. 나머지 고난도 컷** | 컷 9(입자 해체), 컷 1(각석) | ~20% | 실패율이 높다. 대안 연출 판단을 늦게 할수록 손해 |
| **3. 최소 완성본** | 컷 3, 4, 8 | ~30% | 여기까지 나오면 **6컷 30초짜리 완성품**이 손에 들어온다 |
| **4. 확장분** | 컷 5(운해), 6 본작업, 7(석남사) | ~20% | 45초로 늘리는 구간. 크레딧이 부족하면 생략 가능 |
| 예비 | 마음에 안 드는 컷 재생성 | ~10% | 반드시 남겨둘 것 |

### 최소 완성본 = 6컷 30초

크레딧이 모자라도 이 6컷만 있으면 출품 가능한 완결 영상이 된다. (공고 규격 15~60초 충족)

`컷1 암각화` → `컷2 탈출` → `컷3 대곡천+관광객` → `컷4 억새` → `컷8 간절곶 일출` → `컷9 환원`

암각화 → 탈출 → 울주 통과 → 마무리라는 서사가 그대로 살고, 울주 랜드마크도 3곳 들어간다. **컷 5·6·7은 있으면 좋은 확장이지 필수가 아니다.**

### 1단계에서 확인할 것

컷 2와 컷 6을 나란히 놓고 본다.
- 같은 고래로 보이는가? → 예: 전이 구조 그대로 진행
- 다른 개체로 보이는가? → **3절 고정 블록을 더 길게 쓰고 재시도.** 두 번째도 실패하면 전이를 포기하고 처음부터 끝까지 "암각화 질감 고래"로 통일한다. 그래도 영상은 성립한다 (변화 대신 여정이 축이 된다)

기획을 지키려다 크레딧을 다 태우는 게 최악이다. **2번 시도해서 안 되면 기획을 굽힌다.**
