...
| 단계 | 내용 | 배분 | 왜 이 순서인가 |
|---|---|---:|---|
| **1. 일관성 테스트** | 컷 1(암각화 상태) + 컷 6(생명체 상태) | ~15% | **여기서 막히면 기획 자체를 바꿔야 한다.** 두 컷의 고래가 같은 개체로 안 보이면 전이 구조를 포기하고 단일 형태로 간다 |
| **2. 고난도 컷** | 컷 2(폭발·풀백), 컷 9(입자 해체) | ~25% | 실패율이 가장 높다. 안 되면 대안 연출로 바꿔야 하는데, 그 판단을 늦게 할수록 손해 |
