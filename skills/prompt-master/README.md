...
**Works with:** Claude, ChatGPT, Gemini, reasoning-native models, MiniMax, Cursor, Claude Code, GitHub Copilot, Windsurf, Cline, Bolt, v0, Lovable, Devin, Perplexity, Midjourney, DALL-E, Stable Diffusion, ComfyUI, Sora, Runway, ElevenLabs, Zapier, Make, and any AI tool you throw at it.
...
|------|----------|--------------------------|
| **Claude** (Fable 5, Opus 5, Sonnet 5, Haiku 4.5, 4.x) | Reasoning LLM | Removes padding, adds XML structure, specifies length, keeps thinking/effort/prefill out of the prompt body where they no longer work |
| **ChatGPT / GPT** | Reasoning LLM | Output contract, verbosity control, completion criteria |
| **Gemini** | Reasoning LLM | Grounding anchors, citation rules, format locks |
| **Reasoning-native models** | Thinking LLM | Short clean instructions only — never adds CoT (they think internally) |
| **Ollama** | Local LLM | Asks which model is loaded, includes system prompt for Modelfile |
| **Qwen** | Open-weight LLM | Chat template format, thinking vs non-thinking mode detection |
...
| **MiniMax** | Reasoning LLM | Temperature clamping, thinking tag control, structured output optimization |
...
| **Chain of Thought** | Forces step-by-step reasoning for logic tasks — never applied to a reasoning-native model |
...
| 27 | **Adding CoT to a reasoning-native model** | "think step by step" sent to a model that reasons internally | Removed, reasoning-native models think internally and CoT instructions degrade output |
...
- **2.0.0** — Refreshed the Claude routing to the Claude 5 generation (Fable 5 / Opus 5 / Sonnet 5 / Haiku 4.5, with 4.x still selectable), verified 2026-08-21. Moved thinking, effort, and prefill from prompt-text advice to request-parameter advice, since prose can no longer set them. Split per-tool routing out of `SKILL.md` into `references/tool-routing.md`, cutting what loads on every trigger by roughly 60%. De-pinned version numbers in the non-Claude sections and added a verification-status note. Resolved three contradictions between `SKILL.md`, `templates.md`, and this README. Patterns 26/27/36 de-pinned; added patterns 38-40 for superseded techniques.
