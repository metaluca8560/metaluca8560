...
| [M — Claude Task Brief](#template-m--claude-task-brief) | Complex, multi-step, or agentic task on Claude |
...
**Important:** Use this template ONLY on models that do not reason natively and expose no thinking or effort setting. Do NOT use it on reasoning-native models — the OpenAI o-series, DeepSeek-R1, Qwen thinking modes, or any current Claude model. They reason internally, and CoT instructions degrade their output. Where a model exposes a thinking or effort control, set that on the request instead of writing reasoning scaffolding into the prompt. Check the model's section in tool-routing.md before reaching for this template.
...
**When NOT to use:**
- Any reasoning-native model, current Claude models included (they think internally — adding CoT hurts)
...
- Examples are the strongest signal in a prompt — the model matches their length, tone, and structure. Vary them deliberately rather than repeating one shape.
...
- **Midjourney**: Comma-separated descriptors, not prose. Add `--ar` and `--style` at the end; confirm the current `--v` value before pinning a version.
...
## Template M — Claude Task Brief

*Use for any complex, multi-step, or agentic task on Claude — claude.ai, the API, or Claude Code. Current Claude models read prompts literally and produce narrow output when context is missing. This template front-loads everything so the first turn is the only turn.*
...
**Thinking depth** — adaptive thinking calibrates itself, so say nothing by default. Depth is properly controlled by `effort` on the request, not by prompt text. The only prose levers that do anything:
...
Never write "think step by step" or a token budget into a Claude prompt.
...
**When to use:** any current Claude model on any surface — claude.ai, API, Claude Code — when the task is complex, multi-file, ambiguous, or agentic. Not needed for simple one-shot tasks.

**On Claude Fable 5:** keep the Objective, Context, Target State, Scope, Constraints, and Acceptance Criteria blocks and drop any step-by-step choreography. Fable 5 degrades when over-specified — give it the outcome and the verification bar, not the method.
